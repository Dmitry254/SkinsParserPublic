...
def parser():
    ...
    for item2 in data2:
        ...
        for item3 in data3:
            if item2['market_hash_name'] == item3['market_hash_name']:
                for item1 in data1:
                    if item2['market_hash_name'] == item1[0] == item3['market_hash_name']:
                        ...
                        with_tm_avg = float(item3['avg_price']) / float(item2['lowest_price'])
                        with_tm_price = float(item3['price']) / float(item2['lowest_price'])
                        tm_to_bit = float(item1[1]['max_price']) / float(item3['price'])
                        if with_avg > 1.3:
                            item_buy = (f"{item2['market_hash_name']} с ценой {item2['lowest_price']},"
                                        f" средней ценой {item2['recent_sales_info']['average_price']},"
                                        f" и автопокупкой {item1[1]['max_price']}. На ТМ цена {item3['price']},"
                                        f" а средняя цена {item3['avg_price']}")
                            print(item_buy)
...
def check_place(name, price):
    ...
    api = requests.get(url)
    data = json.loads(api.text)
    place = int(data['data']['expected_place_in_queue'])
...
def get_float(item_id):
    my_secret = google_code
    my_token = pyotp.totp.TOTP(my_secret).now()
    url = f"https://bitskins.com/api/v1/get_specific_items_on_sale/?api_key={api_bit}&code={my_token}&item_ids={item_id}"
    api = requests.get(url)
    data = json.loads(api.text)
